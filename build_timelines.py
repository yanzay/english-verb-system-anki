...
# ── SVG primitives ──────────────────────────────────────────────────────
HEADER = '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 80" \
...
<style>
  ...
  .label {{ fill:#6b7280; font-size:10px; letter-spacing:0.04em; }}
  .now   {{ fill:#dc2626; font-weight:700; letter-spacing:0.04em; }}
  .event {{ fill:#1d4ed8; }}
  .arrow {{ fill:#1d4ed8; }}
...
def dot(x: int, y: int = 40, label: str | None = None) -> str:
...
def dur(x1: int, x2: int, y: int = 40, label: str | None = None) -> str:
...
def arrow_dur_to(x: int, y: int = 40, label: str | None = None) -> str:
